...
static int ft_reduction(int number, t_list *flags, int *length)
{
    ...
    if (!flags->null && !flags->minus && ((flags->width - flags->accur > 0) || (width > 0)))
    {
...
static int ft_output(int number, t_list *flags, int *length, int flag)
{
    ...
    if ((accur > 0) || ((flags->null || (flags->indent == 1 && !flags->dot)) && width > 0))
    {
...
static void ft_close(int number, t_list *flags, int *length, int n)
{
    ...
    if (flags->minus && ((flags->width > socr) || (flags->width > flags->accur)))
    {
        if (socr > flags->accur)
